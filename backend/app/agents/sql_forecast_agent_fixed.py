...
class SQLForecastAgent:
    def __init__(self, groq_api_key: str, schema_info: str, db_conn, forecaster, chart_gen, table_mappings: Dict[str, str]):
        ...
        self.llm = self._get_working_llm(groq_api_key)
    ...
    def _get_working_llm(self, groq_api_key: str):
        ...
        for model in models:
            try:
                ...
                if "4" in test_response.content or "four" in test_response.content.lower():
                    print(f"✅ Model {model} is working!")
                    return llm
                else:
                    print(f"❌ Model {model} failed basic test")
        ...
        print("Using fallback model: llama3-70b-8192")
        return ChatGroq(
    ...
    def setup_prompts(self):
        ...
        # SUPER SIMPLE PROMPT - Just the essentials
        self.master_prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content=f"""You are a data analyst. Answer questions about the Northwind database.

Database Schema:
{self.schema_info}

Tables: Products, Orders, Customers, Categories, Order Details, Employees

IMPORTANT: Always answer the user's question. Never say "clarify" or "no specific question".

When asked about products, sales, or performance, generate SQL and provide insights.
...
{{
    ...
    "visualization": "text",
    "response_guidance": "Your analysis and insights",
    "response_type": "text"
}}

Example for "What are our top selling products?":
{{
    "intent": "query",
    "sql_query": "SELECT p.ProductName, SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) as TotalSales FROM \"{order_details_table}\" od JOIN \"{products_table}\" p ON od.ProductID = p.ProductID GROUP BY p.ProductID, p.ProductName ORDER BY TotalSales DESC LIMIT 5",
    "visualization": "text",
    "response_guidance": "Analysis of top selling products by total sales revenue",
    "response_type": "text"
}}"""),
    ...
    def analyze_intent(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            # Test the LLM first with a simple prompt
            test_prompt = f"Question: {state['user_query']}\n\nAnswer with JSON containing sql_query and insights."
            print(f"Using simplified prompt: {test_prompt}")
