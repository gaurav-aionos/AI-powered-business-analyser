...
class SQLForecastAgent:
    ...
    def setup_prompts(self):
        ...
        # SIMPLIFIED PROMPT - Much clearer and direct
        self.master_prompt = ChatPromptTemplate.from_messages([
            SystemMessage(content=f"""You are a data analyst for the Northwind database. 

Your task: Answer user questions by generating SQL queries and providing insights.

Database Schema:
{self.schema_info}

Table Names (use quotes for spaces):
...
CRITICAL RULES:
1. ALWAYS answer the user's question - never say "clarify" or "no specific question"
2. ALWAYS generate a meaningful SQL query
3. Default to "text" visualization for insights
4. Use "table" visualization ONLY when user explicitly asks for tables
...
{{
    ...
    "visualization": "text",
    "response_guidance": "Your insights about what the data shows",
    "response_type": "text"
}}
...
Answer: {{
    ...
    "visualization": "text",
    "response_guidance": "Analysis of top selling products based on total sales revenue",
    "response_type": "text"
}}

Remember: Generate SQL and provide insights. Never ask for clarification."""),
